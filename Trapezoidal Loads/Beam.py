...
from constructores.TrapezoidalLoad import TrapezoidalLoad
...
class Beam:
    ...
    def add_support(self, support_type, pos):
        if pos < 0 or pos > self.L:
            ...
            self.node_id += 1
        if len(self.supports) > 1 and any(support.type == "fixed" for support in self.supports):
    ...
    # agregar carga trapezoidal
    def add_trapezoidal_load(self, start_pos, end_pos, start_load, end_load, d="down"):
        if d == "down":
            start_load = -start_load  # signo carga (positivo hacia arriba, negativo hacia abajo)
            end_load = -end_load  # signo carga (positivo hacia arriba, negativo hacia abajo)
        objeto = TrapezoidalLoad(start_pos, end_pos, start_load, end_load, d, [self.node_id, self.node_id + 1])
        self.cargas.append(objeto)
        if objeto.a_d == "ascending":
            self.nodes.append(Node("trapezoidal_load_min", start_pos, self.node_id, objeto))
            self.node_id += 1
            self.nodes.append(Node("trapezoidal_load_max", end_pos, self.node_id, objeto))
            self.node_id += 1
        elif objeto.a_d == "descending":
            self.nodes.append(Node("trapezoidal_load_max", start_pos, self.node_id, objeto))
            self.node_id += 1
            self.nodes.append(Node("trapezoidal_load_min", end_pos, self.node_id, objeto))
            self.node_id += 1
    ...
    def equivalent_loads(self):
        for carga in self.cargas:
            if type(carga).__name__ == "PointLoad":
                ...
                self.eq_cargas.append(PointLoad(carga.pos, load, carga.d))  # , self.node_id
            elif type(carga).__name__ == "TrapezoidalLoad":
                if carga.a_d == "ascending":
                    # rectangulo
                    load = carga.start_load * carga.length
                    self.eq_cargas.append(PointLoad(carga.rectangle_pos, load, carga.d))  # , self.node_id
                    # triangulo
    ...
    def draw_loads(self):
        for carga in self.cargas:
            ...
            # dibujar carga trapezoidal
            elif type(carga).__name__ == "TrapezoidalLoad":
                arrow_h_min = self.L / 17.5
                arrow_h_max = self.L / 8
                h_factor = (arrow_h_max - arrow_h_min) / carga.length
                arrow_w = self.L / 380
                head_w = self.L / 80
                text_dy_min = arrow_h_min + (arrow_h_min / 2)
                text_dy_max = arrow_h_max + (arrow_h_max / 5)
                if isinstance(carga.length, int):
                    if carga.a_d == "ascending":
                        h = arrow_h_min
                        for i in range(carga.start_pos, carga.end_pos + 1):
                            ax.arrow(i, h, 0, -h, width=arrow_w, head_width=head_w, length_includes_head=True, color="#000000", linewidth=0.5, zorder=3)
                            h += h_factor
                        ax.plot((carga.start_pos, carga.end_pos), (arrow_h_min, arrow_h_max), color="#000000", linewidth=1.3, zorder=3)
                        ax.text(carga.start_pos, text_dy_min, f"{abs(carga.start_load)} kN/m", horizontalalignment="center", verticalalignment="center", fontsize=6, zorder=3)
                        ax.text(carga.end_pos, text_dy_max, f"{abs(carga.end_load)} kN/m", horizontalalignment="center", verticalalignment="center", fontsize=6, zorder=3)
                    elif carga.a_d == "descending":
                        h = arrow_h_max
                        for i in range(carga.start_pos, carga.end_pos + 1):
                            ax.arrow(i, h, 0, -h, width=arrow_w, head_width=head_w, length_includes_head=True, color="#000000", linewidth=0.5, zorder=3)
    ...
    def draw_moment(self, nodes=False, max_value=False, decimals=3):
        ...
        if max_value == True:
            ...
            def max_abs(M):
                ...
                indice_min_M, valor_min_M = min(enumerate(M), key=lambda x: x[1])
                if abs(valor_min_M) > abs(valor_max_M):
                    return indice_min_M, valor_min_M
                else:
                    return indice_max_M, valor_max_M

            indice_abs_max_M, valor_abs_max_M = max_abs(M)
            x_max = x_ex[indice_abs_max_M]
            M_max = valor_abs_max_M
